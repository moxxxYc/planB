...
| 区域 | 位置 | 用途 |
|---|---|---|
| 三段球机 | 顶部横排 | 战备区、发球区、出兵区，是核心机器系统 |
| 自动战场 | 中部主视觉 | 长战场、基地、单位、投射物、接触标记、迷你地图 |
| HUD | 底部 | 战况、基地、资源、部队、下次出兵、战备统计、遗物/科技/工事摘要 |
...
| `spawn` | 当前球直接转成 unit ball，进入右侧出兵区 |

launch 球如果掉出顶部区域，仍会被当作丢失处理，并触发原有 miss 回收类建筑、遗物、科技、球标签。
...
战备区位于顶部左侧，负责把球转成资源、法术或升级。当前可见槽位顺序是：

1. 金币
2. 法术
3. 升级

`spawn` 和 `special` 槽仍在内部状态里保留给统计、调试和旧构筑兼容，但当前默认可见战备区不显示它们。

战备槽不是固定等宽，而是按 `widthWeight` 加权布局。奖励或科技改变权重后，视觉 plate 和 Matter sensor 会一起重建，确保实际命中面积和画面一致。
...
- 金币、法术、升级都计入非出兵连击。
- 连续 3 次非出兵战备结果会创建 1 个出兵标记。
- 法术和升级各给 1 点基础研究。
- 每 4 点基础研究回流进度会创建 1 个出兵标记。
- 科技、建筑、遗物还可以额外把金币、法术、丢失、挡板弹回转成出兵标记或复制。

### 4.3 出兵区

只有发球区命中 `发兵` 后，unit ball 才会进入出兵区。出兵区有 5 个当前种族的单位槽，每个槽绑定一个具体单位，不再随机从单位池直接抽。
...
- 默认开放速度为旧版 1/5，完整开放需要约 2 个阶段时长；单阶段结束时约开放到前 3 槽。
- 高阶绞盘建筑会加快有效开放速度。
...
| `spawn-mark` | 在发球区命中发兵时，让进入出兵区的球 value +1 |
| `copy-mark` | 在发球区命中发兵时，给下一次具体出兵增加 1 个复制 |
| `heavy` | 在发球区命中发兵时，让 unit ball value +1 |
| `recycle` | 如果 launch 球丢失，按球 value 创建出兵标记 |
...
标签在发球区命中发兵时会被翻译：`spawn-mark` 和 `heavy` 变成 value，`copy-mark` 变成复制状态；只有 `recycle`、`split+` 会作为可继续传递的标签保留。
...
1. 固定生成 1 张发球区蓝图、1 张战备区蓝图、1 张出兵区蓝图。
2. 每个仓室先从本仓可安装或可升级建筑中抽 1 张。
3. 本仓建筑没有可用项后，再从本仓未拥有遗物或未解锁科技中抽取。
4. 本仓永久构筑都不可用后，用本仓可重复旧版数值奖励兜底。
5. 已拥有且不可重复的遗物不会再出现。
6. 已满级建筑不会再出现；仓室建筑位满时，不给新的同仓建筑，但允许已有建筑升级。
7. 已解锁科技不会再出现。
8. 奖励界面可以花 10 金刷新一次。
9. 每个奖励界面最多刷新一次。
10. 金币不足时不能刷新。
...
旧版奖励被归入三仓蓝图池，作为同仓可重复兜底，也可被调试预设直接应用。
...
遗物是三仓蓝图池中的不可重复构筑。每个遗物只能获得一次，已拥有后不会再进入阶段奖励。

| 遗物 | 类型 | 效果 |
|---|---|---|
| 逆熵保险丝 | 事件遗物 | 每 3 次 launch 球丢失，储存 1 个出兵标记 |
...
| 中央校准学 | 战备学 | 3 | 发兵路线权重 +15% |
| 转译矩阵 | 战备学 | 4 | 每 3 次非发兵战备结果，研究 +3 并储存 1 个出兵标记 |
...
阶段结束奖励现在固定给三张构筑蓝图：发球区、战备区、出兵区各一张。默认自然流程会优先把可安装或可升级的仓室建筑放进对应仓的蓝图位，因此玩家不打开构筑抽屉也能持续改造三段机器。

构筑抽屉保留为辅助入口：查看当前遗物、科技、工事摘要，使用阶段工具，用金币购买或升级出兵区工事，用研究点购买科技。它不再承担阶段主构筑选择。
...
| 建筑 | 仓室 | 效果 | 当前入口 |
|---|---|---|---|
| 分裂联箱 | 发球区 | split 额外再投同值球，等级越高额外球越多 | 阶段蓝图 / 调试预设 |
| 回收缓冲 | 发球区 | launch 球丢失时储存出兵标记，数量等于等级 | 阶段蓝图 / 调试预设 |
| 铸币导槽 | 战备区 | 每若干次金币命中储存 1 个出兵标记；等级越高阈值越低 | 阶段蓝图 / 调试预设 |
| 电弧回声 | 战备区 | 法术命中后，下一次出兵复制 +等级 | 阶段蓝图 / 调试预设 |
| 溢流孵化器 | 出兵区 | 任一单位入队时，右侧相邻单位槽 +等级 * 入队次数 进度 | 阶段蓝图 / 构筑抽屉 / 调试预设 |
| 高阶绞盘 | 出兵区 | 加快高阶挡板开放 | 阶段蓝图 / 构筑抽屉 / 调试预设 |
| 队列输送带 | 出兵区 | 单位入队时，本批单位获得更短释放间隔和 `queue-burst` 标签 | 阶段蓝图 / 构筑抽屉 / 调试预设 |
...
出兵标记是当前最重要的回流资源。它不会直接生成单位，而是在下一次发球区 `发兵` 创建 unit ball 时兑现。
...
| 预设 | 种族 | 核心幻想 | 关键配置 |
|---|---|---|---|
| 虫群爆兵 | 虫群 | 多球、分裂、溢流、低阶密度 | 分裂联箱、溢流孵化器、队列输送带、棱镜弹匣、双球投放、爆兵储备、虫群孵化、扩大发兵路线 |
| 法术复制 | 虫群 | MAGIC 命中创造复制，下次出兵放大 | 电弧回声、分裂联箱、棱镜弹匣、法术回声石、法术复制、魔法过载、扩大发兵路线 |
...
| 逆风修复 | 虫群 | 丢失、非出兵、挡板弹回都能回流 | 回收缓冲、铸币导槽、溢流孵化器、队列输送带、逆熵保险丝、挡板动量芯、黄金槽扩张、增援鼓点 |
...
- 存在发球遗物、战备科技、出兵工事三类构筑面。
- 状态持久化 `launchRelics`、`decisionTechs`、`unitStructures`、`researchPoints`、`ballTags`、`phaseToolStock`、`chainHistory`、`buildArchetypeHint`。
- 阶段为中短局，自动投球够快，挡板缩短速度为旧版 1/5，阶段内不再全开。
- 阶段奖励固定给三仓蓝图三选一，且自然流程能通过蓝图形成构筑。
- 至少 5 套 debug 预设且能分辨构筑身份。
- 探针覆盖 launch -> standby -> unit slot -> queue -> deploy -> combat impact。
...
|---|---|---|---|---|
| 遗物 | 蓝图可达 | 不通过抽屉购买 | 可通过预设 | 同仓建筑不可用后进入对应仓蓝图池 |
| 旧版数值奖励 | 蓝图兜底可达 | 不通过抽屉购买 | 可通过预设 | 包含槽宽、球数、法术、精英、种族奖励，按仓室归类 |
| 研究科技 | 蓝图可达 / 研究点购买可达 | 可达 | 可通过 action | 构筑抽屉保留研究点购买入口 |
| 出兵区工事 | 蓝图可达 / 金币购买可达 | 可达 | 可通过预设 | 金币购买是辅助入口，阶段蓝图是主入口之一 |
| 发球/战备建筑 | 蓝图可达 | 不可直接购买 | 可通过预设/API | 默认自然流程会优先给同仓可安装或可升级建筑 |
| 阶段工具 | 金币购买可达 | 可达 | 可通过 action | 每阶段中段只可买 1 个 |
| 构筑身份视觉 | play 模式隐藏 | 无 | debug 模式显示 | 不影响规则，只帮助验证 |
| SPECIAL 槽 | 默认不可见 | 无 | 内部逻辑保留 | 当前不是可见战备槽 |
...
- 这个辅助不会直接凭空生成单位，仍会经过发兵、unit ball、slot progress、queue、deploy。
...
- 默认奖励已经改成三仓蓝图三选一；需要继续关注三仓候选池权重是否让构筑方向过早收敛。
- SPECIAL 仍在内部类型中存在，但不是当前可见玩法。
- 阶段工具、研究科技购买和出兵工事金币购买都在构筑抽屉中，默认 play 画面较干净；这些入口是辅助，不是阶段主构筑选择。
...
- `src/data/slots.ts`：战备槽定义和初始权重。
- `src/data/phases.ts`：六个阶段、敌方目标和刷怪时间。
- `src/data/pacing.ts`：当前节奏预设。
- `src/data/rewards.ts`：归入三仓蓝图池的旧版可重复数值奖励。
...
- `src/systems/BuildProbeSystem.ts`：五套 debug 构筑探针和自然三仓蓝图奖励探针。
