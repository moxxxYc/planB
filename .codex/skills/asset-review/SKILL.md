...
This skill is migrated from `/Users/yang/Projects/gstack-game/skills/asset-review`. Preserve the original gstack-game method, rubrics, and game-domain judgment, but run it as a Codex project skill on macOS:

- Use repository-local files and Codex tools. Prefer `rg`, `find`, `sed`, `ls`, and direct file reads.
- Resolve this skill's bundled material relative to `.codex/skills/asset-review/`.
...
Read the referenced files from `.codex/skills/asset-review/references/` only when this section names them or the review needs that rubric. Do not scan home-directory skill stores.


Read ALL reference files now:
- `references/gotchas.md` — Codex-specific failures, anti-sycophancy, 4 forcing questions
...
PREV_ASSET=$(ls -t docs/gstack-artifacts/*-asset-review-*.md 2>/dev/null | head -1)
[ -n "$PREV_ASSET" ] && echo "Prior asset review: $PREV_ASSET"
GDD=$(ls -t docs/gdd.md docs/*GDD* 2>/dev/null | head -1)
[ -n "$GDD" ] && echo "GDD: $GDD"
ENG_REVIEW=$(ls -t docs/gstack-artifacts/*-game-eng-review-*.md 2>/dev/null | head -1)
[ -n "$ENG_REVIEW" ] && echo "Eng review: $ENG_REVIEW"
VISUAL_QA=$(ls -t docs/gstack-artifacts/*-game-visual-qa-*.md 2>/dev/null | head -1)
...
Write to `docs/gstack-artifacts/{user}-{branch}-asset-review-{datetime}.md`. Supersedes prior if exists.
