...
Structure raw game ideas into a validated concept through interactive questioning. Work through each phase one question at a time by asking the user directly. This is a brainstorming skill — almost every decision is ASK.
...
**If web search is unavailable:** Note: "web search unavailable — proceeding with in-distribution knowledge only." Move directly to Phase 2.
...
Present to the user by asking the user directly:
...
**STOP.** Work through each loop level ONE AT A TIME by asking the user directly. If the user can't describe the micro-loop in 15 seconds, the concept needs more work.

**After all three loops are defined, confirm by asking the user directly:**
...
Then generate 2-3 concepts using the chosen technique(s). Present each by asking the user directly:
...
These 6 questions are asked **ONE AT A TIME** by asking the user directly at appropriate points during the session. Do not skip them. Do not soften them. Do not batch.
