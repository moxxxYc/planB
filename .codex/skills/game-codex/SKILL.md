...
This skill is migrated from `/Users/yang/Projects/gstack-game/skills/game-codex`. Preserve the original gstack-game method, rubrics, and game-domain judgment, but run it as a Codex project skill on macOS:

- Use repository-local files and Codex tools. Prefer `rg`, `find`, `sed`, `ls`, and direct file reads.
- Resolve this skill's bundled material relative to `.codex/skills/game-codex/`.
...
Read the referenced files from `.codex/skills/game-codex/references/` only when this section names them or the review needs that rubric. Do not scan home-directory skill stores.
...
- `references/gotchas.md` — Codex-specific adversarial mistakes, anti-sycophancy protocol, push-back cadence
...
**STOP.** Present attack surface map. One direct user question: "Proceed with systematic scan of [N] attack surfaces?"
...
**STOP.** One finding per direct user question. User confirms, disputes, or accepts risk.
...
direct user question: "What decision do you want me to challenge? Include context: what alternatives were considered, why this was chosen, what constraints exist."
...
**STOP.** Present verdict. One direct user question with verdict and recommended action.
...
direct user question: "What are you stuck on? What have you already tried? What constraints can't change?"
...
**STOP.** Present recommendation. One direct user question.
...
Write to `docs/gstack-artifacts/{user}-{branch}-codex-review-{datetime}.md`. Supersedes prior if exists.
