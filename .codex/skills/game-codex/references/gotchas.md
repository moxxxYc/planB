# Codex-Specific Adversarial Review Gotchas

Common mistakes Codex makes during adversarial game review. Read these BEFORE starting any review to avoid them.
...
**Why it happens:** Codex tends to analyze code in isolation. Each function gets its own finding, but the connections between functions are where the real exploits live.
...
**Why it happens:** The full taxonomy is loaded and Codex applies all categories regardless of context.
...
**Why it happens:** Codex reads code statically. Race conditions, timing windows, and concurrent access patterns are extremely hard to spot without execution context.
...
**Why it happens:** Codex's default communication style is cautious and hedge-filled.
