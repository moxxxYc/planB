...
LAST_QA=$(ls -t docs/gstack-artifacts/reviews/$SLUG/game-qa-*.json 2>/dev/null | head -1)
...
Write the full QA report (Baseline + Fixes Applied + Final Score + Delta + Bug List) to `docs/gstack-artifacts/{user}-{branch}-qa-report-{datetime}.md`. If prior QA report exists, include `Supersedes: {prior}`.
