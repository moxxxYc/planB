...
This skill is migrated from `/Users/yang/Projects/gstack-game/skills/game-ux-review`. Preserve the original gstack-game method, rubrics, and game-domain judgment, but run it as a Codex project skill on macOS:

- Use repository-local files and Codex tools. Prefer `rg`, `find`, `sed`, `ls`, and direct file reads.
- Resolve this skill's bundled material relative to `.codex/skills/game-ux-review/`.
...
PREV_UX=$(ls -t docs/gstack-artifacts/*-ux-review-*.md 2>/dev/null | head -1)
...
Review a game's user interface and user experience interactively. Work through each section one issue at a time by asking the user directly. Every recommendation includes WHY and a concrete alternative.
...
Write to `docs/gstack-artifacts/{user}-{branch}-ux-review-{datetime}.md`. Supersedes prior if exists.
