...
export type MvpReadinessStatus = 'proved' | 'partial' | 'missing';
...
export interface MvpReadinessReport {
  ...
  counts: Record<MvpReadinessStatus, number>;
  items: MvpReadinessItem[];
}

export function buildMvpReadinessReport(seed = 170): MvpReadinessReport {
...
export function formatMvpReadinessReport(report: MvpReadinessReport): string {
  const lines = [
    `MVP READINESS ${report.readyForCompletion ? 'READY' : 'NOT_READY'} proved=${report.counts.proved} partial=${report.counts.partial} missing=${report.counts.missing}`,
...
function countStatuses(items: MvpReadinessItem[]): Record<MvpReadinessStatus, number> {
  return items.reduce<Record<MvpReadinessStatus, number>>((counts, item) => {
